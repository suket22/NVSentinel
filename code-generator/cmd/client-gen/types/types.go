...
Origin: https://github.com/kubernetes/code-generator/blob/v0.34.1/cmd/client-gen/types/types.go
*/
...
// ProtobufPackage contains the Go package containing protobuf stubs.
type ProtobufPackage struct {
	Alias   string
	Package string // The Go import-path
}

func NewProtobufPackage(protoBase string, groupPkgName string, version string) ProtobufPackage {
	return ProtobufPackage{
		Alias:   "pb",
		Package: path.Join(protoBase, groupPkgName, version),
	}
}

func (p ProtobufPackage) ImportLines() []string {
	return []string{fmt.Sprintf("%s \"%s\"", p.Alias, p.Package)}
}

func (p ProtobufPackage) ServiceClientConstructorFor(protoType string) string {
	return fmt.Sprintf("%s.New%sServiceClient", p.Alias, protoType)
}
